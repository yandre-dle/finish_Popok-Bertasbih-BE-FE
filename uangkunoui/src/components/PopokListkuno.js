...
import PopokItemkuno from './PopokItemkuno';

class PopokListkuno extends Component {
    ...
    renderListPopok = () => {
        ...
        var listJSXPopok = this.state.searchListPopok.map((item) => {
            ...
            return (
                <PopokItemkuno size={size} popok={item} key={item.id} />
            )
        })
        return listJSXPopok;
    }
...
export default connect(mapStateToProps)(PopokListkuno);